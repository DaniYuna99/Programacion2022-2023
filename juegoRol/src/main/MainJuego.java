...
public class MainJuego {
	...
	private static void jugar(Juego juego) throws JuegoException, JugadorException {
		...
		System.out.println("Le toca al jugador " + juego.getNombreJugadorQueJuega() + ". El dado saca " + juego.getValorDado() + " movimiento(s).");
		...
		juego.proximoJugador(false);
	}

	
	//cambiar modificadores para tests
	public static PlayerType[] cargarJugadores() {
		...
		while (numJugadores < Constantes.NUM_JUGADORES - 1) {
			...
			/*Aquí había un pequeño error (era el <=, que era <) 
			que no dejaba elegir la última clase*/
	...
	//Este método sirve para tests
	public static PlayerType[] cargarJugadoresTest() {
		
		PlayerType[] ordenJugadores = 
			   {PlayerType.ELFO, 
				PlayerType.GUERRERO, 
				PlayerType.MAGO,
				PlayerType.OGRO};
